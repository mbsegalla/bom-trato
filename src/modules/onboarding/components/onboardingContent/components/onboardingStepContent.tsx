...
export function OnboardingStepContent({ state, refreshing, onStateChange, onRefresh }: OnboardingStepContentProps) {
  ...
  if (state.step === 'SELECT_PLAN' && state.organizationId) {
    return (
      <>
        ...
        <div className="mt-8">
          <OnboardingPlanSelection organizationId={state.organizationId} onSelected={onStateChange} />
        </div>
      </>
    );
  }

  if (state.step === 'PAYMENT' && state.organizationId && state.selectedPlanPriceId) {
    return (
      <>
  ...
  if (state.step === 'PAYMENT_PENDING') {
    return (
      <>
        ...
        <p className="mt-3 leading-relaxed text-muted-foreground">
          Recebemos o retorno da Stripe e estamos aguardando a confirmação da assinatura. Não inicie outro pagamento.
        </p>

        <Button type="button" disabled={refreshing} onClick={onRefresh} className="mt-7 min-h-12 rounded-xl">
          {refreshing ? (
            <>
  ...
  if (state.step === 'CREATE_BUSINESS' && state.organizationId) {
    return (
      <>
        ...
        <div className="mt-8">
          <OnboardingBusinessForm organizationId={state.organizationId} onCompleted={onStateChange} />
        </div>
      </>
    );
  }
  ...
  if (state.step === 'BILLING_REQUIRED') {
    return (
      <>
